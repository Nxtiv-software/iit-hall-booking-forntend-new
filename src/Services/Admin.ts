...
export const fetchAdmin1All = async (adminId: string, token: string) => {
  const response = await axios.get(`${BASE_URL}/${adminId}/admin1/all`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  return response.data;
};
