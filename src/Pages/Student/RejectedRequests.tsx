...
import { fetchStudentProfile, fetchStudentRejectedRequests } from "@/Services/Students"

const StudentRejectedRequests = () => {
    ...
    const { data: rejectedRequestData = [], isLoading: requestLoading } = useQuery({
        queryKey: ["rejectedRequests", studentId],
        queryFn: async () => {
            const currentUser = auth.currentUser;
            if (!currentUser) throw new Error("Not authenticated");

            const idToken = await currentUser.getIdToken();
            return fetchStudentRejectedRequests(idToken, studentId!);
        },
        enabled: !!studentId,
        retry: false, 
    });
            ...
            <div className="flex flex-1 flex-col gap-4 p-4">
                <div className="bg-muted/50 min-h-screen flex-1 rounded-xl md:min-h-min p-4">
                    ...
                    <Table>
                        ...
                        <TableBody>
                        {rejectedRequestData.map((request: { id: string; student: { user: { firstName: string; lastName: string } }; venue: { name: string }; status: { name: string }; createdAt: string }) => (
...
export default StudentRejectedRequests